      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden border border-zinc-200">
        <div className="px-6 py-5 border-b border-zinc-200">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-medium text-zinc-400 mb-0.5 uppercase tracking-wide">{transaction.date}</p>
              <h3 className="text-base font-semibold text-zinc-900">{transaction.merchant}</h3>
              <p className={`text-xl font-semibold mt-1 font-mono tabular-nums ${transaction.amount >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-sm text-xs font-medium ${CATEGORY_COLORS[transaction.category]}`}>
                {CATEGORY_LABELS[transaction.category]}
              </span>
            </div>
            <div className="text-right">
              <p className="text-xs text-gray-400 mb-1">Classification</p>
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-sm text-xs font-medium ${src.className}`}>
                {src.label}
              </span>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className={`text-sm font-semibold tabular-nums ${
                transaction.confidence >= 90 ? 'text-zinc-700'
            ...
            <div className="h-1.5 bg-zinc-100 rounded-sm overflow-hidden">
              <div
                className={`h-full transition-all ${
                  transaction.confidence >= 90 ? 'bg-zinc-400'
          ...
          <div className="flex flex-col gap-1.5 py-3 px-4 bg-zinc-50 rounded border border-zinc-100">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <ReceiptIcon className="w-4 h-4 text-gray-400" />
                <p className="text-sm text-gray-600">Evidence match</p>
              </div>
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-sm text-xs font-medium ${match.className}`}>
                {match.label}
              </span>
            </div>
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 border border-zinc-300 hover:border-zinc-500 hover:text-zinc-900 text-zinc-600 text-sm font-semibold rounded transition-colors cursor-pointer"
          >
            Recategorise
          </button>
        </div>
      ...
      <div className="bg-white rounded-md border border-gray-200">
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-12 h-12 rounded bg-zinc-100 flex items-center justify-center mb-3">
            <ListIcon className="w-6 h-6 text-zinc-400" />
          </div>
    ...
    <div className="bg-white rounded-md border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs text-gray-400">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-zinc-200 inline-block" />90%+</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-amber-200 inline-block" />70–89%</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-red-200 inline-block" />&lt;70%</span>
        </div>
      </div>
