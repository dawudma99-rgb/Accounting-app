...
/**
 * Pure function. Builds the SA100 final filing preview from a completed
 * TaxSummary and accountant-supplied filing inputs (PAYE, student loan,
 * declaration).
 *
 * No recalculation of income tax or NI is performed — all figures are lifted
 * directly from summary.liability. The only adjustments made here are:
 *   1. Deduct tax paid at source (PAYE) to arrive at net SA liability.
 *   2. Recompute the payment-on-account schedule based on that net figure.
 *   3. Generate filing blockers from missing or incomplete inputs.
 */
export function buildSA100Preview(
  ...
  // ── Take-home based on what's actually owed via SA ───────────────────────────
  const afterTaxProfit  = summary.netProfit - netTaxLiability
  const effectiveTaxRate = summary.netProfit > 0
    ? (netTaxLiability / summary.netProfit) * 100
    : 0
  ...
  if (!utr) {
    blockers.push({
      severity: 'blocking',
      label:    'UTR missing',
      message:  'Unique Taxpayer Reference is required before filing. Add it to the client record.',
    })
  }
  ...
  if (sa103HasBlockingFlags) {
    blockers.push({
      ...
      message:  'The SA103 draft has outstanding action items (unresolved flagged transactions). ' +
                'Resolve them in the Dashboard before treating this return as ready.',
    })
  }
  ...
  if (!declarationConfirmed) {
    blockers.push({
      ...
      message:  'The client must confirm the declaration of truthfulness before the return can be filed.',
    })
  }
  ...
  if (liability.otherIncome > 0 && taxPaidAtSource === 0) {
    blockers.push({
      ...
      message:  `Other income of ${fmt(liability.otherIncome)} is included. If tax was deducted at source ` +
                `(e.g. PAYE on employment income), enter it to calculate the correct net liability.`,
    })
  }
  ...
  if (studentLoanPlan === undefined) {
    blockers.push({
      ...
      message:  'Select the client\'s student loan repayment plan, or "No student loan" if not applicable.',
    })
  }
  ...
  return {
    ...
    selfEmploymentProfit:       summary.netProfit,
    otherIncome:                liability.otherIncome,
    totalIncome:                liability.totalIncome,
